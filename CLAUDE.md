...
- **local-bridge/** — TypeScript MCP server (Express + SSE). Exposes 6 tools (`read_file`, `edit_file`, `write_file`, `bash`, `glob`, `grep`) on the local filesystem. Security via path whitelist (`config.ts` → `security.ts`).
...
- **No auth layer** — Security relies on SSH tunnel. Bridge only listens on localhost.
